#!/usr/bin/env python3
"""Test REFERENCE-*.md generation for Ruby on Rails project (discourse).

Test Ruby support end-to-end.
"""
...
def test_ruby_reference_generation(
    repo_name: str = "discourse",
    components_to_generate: list[str] = ["models", "controllers", "serializers"],
):
    """Test REFERENCE-*.md generation for Ruby components."""
    ...
    print(f"\n{'='*80}")
    print(f"Testing Ruby REFERENCE-*.md Generation: {repo_name}")
    print(f"{'='*80}\n")
    ...
    print(f"   Framework: {grouping_result['framework']}")
    print(f"   Language: {grouping_result['primary_language']}\n")
    ...
    if not target_components:
        ...
        print(f"   Available: {[c['name'] for c in grouping_result['components']]}")
        return

    print(f"2. Selected {len(target_components)} components for documentation:")
    for comp in target_components:
        print(f"   - {comp['name']} ({comp['type']}, {len(comp['files'])} files)")
    print()
    ...
    analyzed_components = []
    for comp in target_components:
        print(f"   - Analyzing {comp['name']} ({len(comp['files'])} files)...", end=" ", flush=True)
        ...
        modules = api.get("total_modules", 0)
        methods = api.get("total_methods", 0)
        total_apis = classes + modules + methods

        print(f"✓ {total_apis} APIs ({classes}c+{modules}m), {result.get('api_coverage', 0)}% coverage")

        # Show any warnings
        if api.get("warnings"):
            print(f"      ⚠️  {len(api['warnings'])} files had parsing errors")
    ...
    for file_path in generated_files:
        ...
        if has_classes or has_modules:
            print(f"  ✓ API elements documented")
        else:
            print(f"  ⚠️  No API elements found")
    ...
    total_apis = sum(
        ac.get("api", {}).get("total_classes", 0) +
        ac.get("api", {}).get("total_modules", 0) +
        ac.get("api", {}).get("total_methods", 0)
        for ac in analyzed_components
    )
...
if __name__ == "__main__":
    # Test on discourse (Ruby on Rails) - focus on smaller components
    test_ruby_reference_generation(
        repo_name="discourse",
        components_to_generate=["helpers", "mailers", "queries"],  # Smaller components
    )
